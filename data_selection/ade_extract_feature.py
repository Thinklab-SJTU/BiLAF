...
from datasets.custom_dataset import ADEReturnIndexDataset
...
def extract_features(args):
    ...
    dataset = ADEReturnIndexDataset(args.data_root, data_transform)
    ...
    features, ids = validate_network(data_loader, model, args.n_last_blocks, args.avgpool_patchtokens, args.maxpool_patchtokens)
    ...
    if args.avgpool_patchtokens:
        np.save(os.path.join(args.output_dir, "ade_%s_%s_avgpool_features.npy"%(args.framework, args.arch)), features)
        with open(os.path.join(args.output_dir, "ade_%s_%s_avgpool_names.json"%(args.framework, args.arch)), "w") as file:
            json.dump(ids, file)
    elif args.maxpool_patchtokens:
        np.save(os.path.join(args.output_dir, "ade_%s_%s_maxpool_features.npy"%(args.framework, args.arch)), features)
        with open(os.path.join(args.output_dir, "ade_%s_%s_maxpool_names.json"%(args.framework, args.arch)), "w") as file:
            json.dump(ids, file)
    else:
        np.save(os.path.join(args.output_dir, "ade_%s_%s_features.npy"%(args.framework, args.arch)), features)
        with open(os.path.join(args.output_dir, "ade_%s_%s_names.json"%(args.framework, args.arch)), "w") as file:
            json.dump(ids, file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default="features/ADE20K", help='Path to save extracted features')
    parser.add_argument('--data_root', default="/data/dataset/ADE20K/ADEChallengeData2016/images/training", type=str, help="root dir of Pascal VOC")
